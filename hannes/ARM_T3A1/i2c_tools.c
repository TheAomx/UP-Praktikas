...
unsigned char i2c_rbit(void) {
	...
	AT91C_BASE_PIOA->PIO_SODR = SCL;				// SCL -> 1
	if (AT91C_BASE_PIOA->PIO_PDSR & SDA)
		bit = 1;		// Pegel von SDA lesen
	else
		bit = 0;										//	und bit zuweisen
...
void i2c_wbit(unsigned char bit) {
	if (bit)
		AT91C_BASE_PIOA->PIO_SODR = SDA;		// Ausgabe von bit: High
	else
		AT91C_BASE_PIOA->PIO_CODR = SDA;			//                  Low
	AT91C_BASE_PIOA->PIO_SODR = SCL;				// SCL -> 1
	{
		asm("NOP");
		asm("NOP");
...
unsigned char i2c_start(unsigned char sladr) {
	...
	AT91C_BASE_PIOA->PIO_CODR = SCL;				// SCL -> 0
	if (!i2c_wbyte(sladr))
		return 0;				// falls Slave antwortet -> raus, ACK = 0
...
unsigned char i2c_rbyte(unsigned char ACK) {
	...
	for (int i = 7; i > -1; i--) {		// Jedes Bit seriell senden
		if (i2c_rbit())
			byte |= (1 << i);
	}
	i2c_wbit(ACK);
